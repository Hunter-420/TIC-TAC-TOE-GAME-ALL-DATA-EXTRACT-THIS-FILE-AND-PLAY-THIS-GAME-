...
case 'H': 
loading(); 
gamePage("HOW TO PLAY?"); howToPlay(); 
pressAkeyToContinue(); 
goto Option; 
break; 
case 'D': 
loading(); 
gamePage("DEVELOPER"); developerInformation(); 
pressAkeyToContinue(); 
goto Option; 
break; 
default: 
loading();
badInput(); 
pressAkeyToContinue(); goto Option; 
break; 
} 
TextColor: 
loading(); 
gamePage("TEXT COLOR"); textColorDisplay(); 
position = optionChoice(); 
switch (position) 
{ 
case '1': 
...
case '8': 
system("color 8"); break; 
case '9': 
system("color 9"); break; 
case 'A': 
system("color A"); break; 
case 'B': 
system("color B"); break; 
case 'C': 
system("color C"); break; 
case 'D': 
system("color D"); break; 
case 'E': 
system("color E"); break; 
case 'F': 
system("color F"); break; 
default: 
loading(); 
badInput();
pressAkeyToContinue(); 
...
{ 
case '1': 
system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,10)"); break; 
case '2': 
system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,20)"); break; 
case '3': 
system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,30)"); break; 
case '4': 
system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,40)"); break; 
case '5':
system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,50)"); break; 
case '6': 
system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,60)"); break; 
case '7': 
system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,70)"); break; 
case '8': 
system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,80)"); break; 
case '9': 
system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,90)"); break; 
case 'A': 
system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,100)"); break; 
default: 
badInput(); 
pressAkeyToContinue(); 
goto ScreenBrightness; 
...
void developerInformation() 
{ 
"This will show the developer information"; FILE *out_developer; 
char ch; 
...
void thankYou() 
{ 
"This will thanks to the user who had played the game"; FILE *out_thank_you; 
char ch; 
int sml = 2; 
...
void pressAkeyToContinue() 
{ 
"Ask user a key to press to move ahead"; printf("\n\n%30s", ""); 
system("pause"); 
} 
void badInput() 
{ 
"Display the bad input information to the user"; gamePage("BAD INPUT"); 
printf("%25sPlease try to given valid input.\n", ""); 
printf("\n%5sTry again with valid input.\n", " "); }
void gameHomeDisplay() 
{ 
...
if (strcmpi(position, "NW") == 0 || strcmpi(position, "NN") == 0 || strcmpi(position, "NE") == 0 || strcmpi(position, "CW") == 0 || strcmpi(position, "CC") == 0 || strcmpi(position, "CE") == 0 ||
strcmpi(position, "SW") == 0 || strcmpi(position, "SS") == 0 || strcmpi(position, "SE") == 0) { 
...
data.time_of_game_play.minutes = data.stop.minutes - data.start.minutes; data.time_of_game_play.seconds = data.stop.seconds - data.start.seconds; 
if (data.time_of_game_play.seconds < 0) 
{ 
data.time_of_game_play.seconds = data.time_of_game_play.seconds + 60; data.time_of_game_play.minutes = data.time_of_game_play.minutes - 1; } 
} 
void writeGameData()
{ 
...
char optionChoice() 
{ 
"This will let user to position a option and return the choiced option"; char position; 
...
printf("%25s%-15s%10s%-15s\n", "", "1 = Blue ", "", "9 = Light Blue "); printf("%25s%-15s%10s%-15s\n", "", "2 = Green ", "", "A = Light Green "); printf("%25s%-15s%10s%-15s\n", "", "3 = Aqua ", "", "B = Light Aqua "); printf("%25s%-15s%10s%-15s\n", "", "4 = Red ", "", "C = Light Red "); printf("%25s%-15s%10s%-15s\n", "", "5 = Purple", "", "D = Light Purple"); printf("%25s%-15s%10s%-15s\n", "", "6 = Yellow", "", "E = Light Yellow"); printf("%25s%-15s%10s%-15s\n", "", "7 = White ", "", "F = Bright White"); printf("%25s%-15s%10s%-15s\n", "", "8 = Gray ", "", ""); 
} 
void screenBrightness() 
{ 
